...
| **demo 应用** | 业务代码 + Vue 前端 + MySQL 接入 + Nginx/压测脚本 | **允许** | 证明内核「真能用」，跑通全链路并验收高可用 |
...
| 单元测试 | JUnit 5 | 仅测试期依赖，内核运行时保持零依赖 |

### 2.2 核心取舍：框架内核零第三方运行时依赖
...
> 成本收益评估：自写 JSON/YAML/HTTP 服务器的收益是「教育价值」，成本是「多写几千行代码且容易埋 bug」。对于框架内核，这笔账是划算的；对于 demo 应用，为了「真能用」，则反过来**允许引入真实依赖**——这正是双轨制的意义。
...
内核保持零依赖；demo 层为了「真能用、高可用」引入真实基础设施：

| 层 | 选型 | 理由 |
| --- | --- | --- |
| 数据库 | MySQL 8 + HikariCP 连接池 | 真实工程级，验证「能用」与多实例高可用 |
| 前端 | Vue 3（Vite 构建） | 现代工程形态，浏览器 F12 可调试全链路 |
| 负载均衡 | Nginx（`upstream` + 健康检查） | 3 实例无状态水平扩展 + 故障转移 |
...
| 测试 | 核心模块有单测，关键链路有集成测试，并发下有正确性测试 |
...
demo 是一套典型的「用户/订单管理」三层 Web 应用，用于证明内核「真能用」并跑通全链路：

```
浏览器（Vue 3 前端，F12 可调试）
   │ HTTP
   ▼
Nginx（负载均衡 + 健康检查）
   ├─► demo 实例 1（MiniSpringBoot 内嵌服务器 :8081）
   ├─► demo 实例 2（:8082）
   └─► demo 实例 3（:8083）
          │ JDBC（HikariCP 连接池）
          ▼
...
- 前端（Vue 3）通过 JSON API 访问后端，F12 里直接看请求/响应/状态码。
...
2. **外部健康检查**：Nginx 定期探测每个实例，挂掉的实例被自动摘除，流量切到存活实例——这就是「故障转移」。

```
        Nginx upstream
   ┌──────┼──────┐
 :8081  :8082  :8083   （任意一个宕机，其余两个接管）
```

因此「3 实例高可用」的完整语义是：**任一实例故障，服务不中断、数据不丢、请求可被其余实例承接。**
...
| 集成测试 | JUnit 5 + 内存库 | Web/Service/DAO 链路 |
...
- Vue 3 + Vite，独立目录 `demo-frontend/`，产物可被内嵌服务器作为静态资源托管，也可 `npm run dev` 独立联调。
