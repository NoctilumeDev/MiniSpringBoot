...
用户删除使用 `UsersPage` 自己拥有的页内非阻塞确认层，不调用 `window.confirm`。确认层只负责收集“取消/确认”意图，真正删除仍只经过既有 `api.deleteUser` 与 DELETE 契约；取消或按 `Escape` 不产生请求。这样既保留不可逆操作的明确停顿，也不让浏览器原生模态框暂停整个页面脚本或把展示层状态泄漏到控制通道。
