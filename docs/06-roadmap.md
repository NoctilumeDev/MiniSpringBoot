...
- **M3 当时的落地边界（主体已于 M7 关闭）**：M3 验收时，`DefaultListableBeanFactory.getEarlyBeanReference` 仍直接返回原始对象，因此「被代理 Bean 同时参与循环依赖」不在当期验收口径内。M7 随后新增 `SmartInstantiationAwareBeanPostProcessor#getEarlyBeanReference` 扩展点，由 `AspectJAutoProxyCreator` 提前生成代理并用 `earlyProxyReferences` 去重；当前实现已不再保留这项主体技术债。切面收集期已注入的裸对象引用无法自动替换，仍作为 D44 显式边界保留。
...
- **中危修复**：M1 注入裁决四路径（core 的 Primary/qualifier 参数裁决 + context 的 resolveByQualifier/resolveByPrimary）遇运行期手动单例（无 BeanDefinition，如 webServer）崩溃——D46 修了可见性没扫齐同族；M2 监听器泛型解析不认子接口（D39 只修父类链）——raw Class 固化形态当轮修复；「带自身类型参数」形态（`SmartListener<E> extends ApplicationListener<E>`）当轮宣称同修、实未修通：上溯丢失 E→实参 的绑定、E 以 TypeVariable 到达解析点解不出，静默退化「接收所有事件」，且行为用例因「无关事件在桥接方法抛 CCE、被 B-4 吞掉、计数碰巧为 0」而**假通过**（用户复审指出后追修）：解析器携带「类型变量→实参」绑定表经接口链与泛型父类逐层代入，四种形态全通（直接实现/子接口固化/带参子接口/泛型父类）；`resolveEventType` 改包级可见供单测直接断言解析结果（约束性锚点，注入旧缺陷实测 3/5 用例必失败——「修复错了测试会不会失败」过验）；M3 properties 按 ISO-8859-1 读取中文必乱码（改 Reader+UTF-8，与 yml 对称）；M4 `writeError` 不查 `isCommitted`（已提交后写 500 文本形成混合体）；M5 多 TransactionManager 时切面按 CHM 遍历序随机取一个（改显式报错）；M6 类级 `@Transactional` 静默无效（`@annotation` 切点补类级命中，`@Target` 补 TYPE）；M7 D30 补偿代理的 init/destroy 回调找不到方法（`invokeNoArgMethod` 沿父类链+接口链回查）；M8 `JsonSerializer` 半写状态（字段名+冒号已写出后取值失败留下 `{"a":,` 非法 JSON——先取值再写名）。
...
- **状态（2026-08-26）**：实现与本机事实自验已完成；冻结源码对象 [`85c2b22`](https://github.com/NoctilumeDev/MiniSpringBoot/commit/85c2b22dfdcb17cd2527f068f85542aca25d694c) 已通过 [`m10-evidence-source-v1`](https://github.com/NoctilumeDev/MiniSpringBoot/tree/m10-evidence-source-v1) 公开，并与主线等价提交 [`582b0f5`](https://github.com/NoctilumeDev/MiniSpringBoot/commit/582b0f53a57844c359724424b869c75088b61b50) 保持同一 tree。规范证据清单为 [`docs/evidence/m10/m10-evidence-manifest.json`](evidence/m10/m10-evidence-manifest.json)。在 VeriTrail Core 0.12 冻结后又执行一次有界故障切换、事务和就绪恢复回放，15 项 HARD 断言全部通过；冻结 Bundle 见 [`docs/evidence/m10/veritrail/bundle`](evidence/m10/veritrail/bundle)，原始 M10 发布坐标为 [`v0.m10`](https://github.com/NoctilumeDev/MiniSpringBoot/releases/tag/v0.m10)。独立归档复核先形成包含事务连接清理修订的 [`v0.m10.1`](https://github.com/NoctilumeDev/MiniSpringBoot/releases/tag/v0.m10.1)，随后又以 fresh-checkout 审计逐项修复八类内核正确性问题并形成 [`v0.m10.2`](https://github.com/NoctilumeDev/MiniSpringBoot/releases/tag/v0.m10.2)。最终零对话上下文审计又确认 Windows PowerShell 5.1 在 `Start-Process -PassThru` 与输出重定向组合下可能丢失子进程退出码；维护修订现在预先保留进程句柄、拒绝未知或非零退出码，并由 Windows CI 覆盖成功与失败路径。旧标签保持不动，88 项回归测试、前端门禁和完整故障切换实跑通过后的当前维护坐标为 [`v0.m10.3`](https://github.com/NoctilumeDev/MiniSpringBoot/releases/tag/v0.m10.3)。复验范围固定为 `IMPORTED_EVIDENCE_AUDIT`，全拓扑生命周期所有权仍为 `NOT_PROVEN`。
...
| D60 | ~~33：REQUIRED 嵌套调用无 rollbackOnly 标记——内层事务 catch 吞掉异常时外层照常提交半途数据~~ **已修复**：`TransactionContext` 绑定连接、rollback-only 与首个失败原因；内层 REQUIRED 参与者抛 Exception/Error 时先标记再原样传播，外层即使捕获异常也会在提交前回滚并抛 `UnexpectedRollbackException`；不影响「仅 REQUIRED，不支持 REQUIRES_NEW/savepoint NESTED」的教学边界 | 嵌套 @Transactional 且外层捕获内层异常时 | 已关闭：约束性用例覆盖内层成功、RuntimeException/Error 被外层捕获、未捕获原异常传播及单次回滚/ThreadLocal 清理 |
| D61 | ~~JDK 代理将 `equals/hashCode` 委托给裸目标，导致 `proxy.equals(proxy) == false`，破坏 Object 基本契约~~ **已修复**：`equals` 采用代理身份比较，`hashCode` 使用代理身份哈希，`toString` 仍保留目标的可读表示；Object 方法不进入业务切点 | 代理进入 Set/Map、做自反比较或与裸目标比较时 | 已关闭：直接断言自反性、代理/目标对称性、不同代理不相等及身份 hashCode |
| D62 | ~~`SunHttpServer` 创建 cached executor 后未保留引用，`stop()` 只停止 JDK `HttpServer`，工作线程池不受生命周期管理~~ **已修复**：服务器显式持有自建 executor；停止、启动失败和启动中断路径统一关闭；重复 start fail-fast，stop 幂等且支持干净重启 | 测试反复启动/停止、应用热重启或 shutdown hook 退出时 | 已关闭：真实 HTTP 请求触发 worker 后断言 executor shutdown/termination，并覆盖重复启动、重复停止与重启隔离 |
| D63 | ~~构造器与 `@Bean` 参数解析在多个 `@Primary` 候选时返回注册顺序中的第一个，而字段/方法注入会拒绝，四种入口语义分裂~~ **已修复**：core 提供唯一的 `DependencyCandidateResolver`，四种注入位置共享“0 个不裁决、1 个选中、多个 fail-fast”的规则 | 重构注册顺序、同时误标两个主实现时 | 已关闭：构造器、工厂方法、字段、方法四条负例均断言错误点名两个冲突 beanName |
| D64 | ~~AOP 代理进入一级缓存后，关闭阶段直接在 JDK 代理上查找 `@Bean(destroyMethod)`；实现类专属销毁方法不在业务接口上时只打印失败、资源未释放~~ **已修复**：容器分别登记对外暴露对象与原始销毁目标，代理不再改变生命周期所有权 | 被切点命中的资源 Bean 通过实现类专属方法释放资源时 | 已关闭：真实上下文确认对外返回 JDK 代理，并断言关闭时原始目标销毁方法恰好执行一次 |
| D65 | ~~配置读取时只保存 `@Bean` 的方法名；同一配置类存在同名重载时，实例化阶段按反射枚举顺序取第一个，可能创建错误返回类型或解析错误参数~~ **已修复**：`BeanDefinition` 保存读取阶段确认的精确 `Method`；旧式仅登记方法名的兼容路径遇到重载时 fail-fast | 配置类以不同 beanName 声明同名重载 `@Bean` 方法时 | 已关闭：真实注解上下文同时实例化无参与带参重载；core 负例确认 name-only 元数据不会静默任选 |
| D66 | ~~`@annotation` 解析对 `Class<?>` 做无运行时校验的泛型强转，`@annotation(java.lang.String)` 会通过构造并把错误推迟到方法匹配阶段~~ **已修复**：加载后先以 `Class#isAnnotation()` 验证，再通过 `asSubclass(Annotation.class)` 固化类型 | 切点表达式误写成存在但不是注解的类名时 | 已关闭：负例确认构造阶段给出“类型不是注解”的可读错误，正例确认真实运行时注解仍正常命中 |
| D67 | ~~自动代理判断与 JDK 代理创建都只读取目标类直接声明的 `getInterfaces()`；子类仅继承父类已实现的业务接口时会被误判为“无接口”，切点即使命中也直接返回裸对象~~ **已修复**：新增唯一的接口解析器，沿类继承和接口继承层级确定性收集完整代理接口；自动代理准入与代理创建共享同一结果 | AOP 目标类通过父类继承业务接口、自己未重复声明 `implements` 时 | 已关闭：真实注解上下文确认子类 Bean 暴露为 JDK 代理、业务调用成功且通知恰好执行一次 |
