...
| D37 | `ConfigFilePropertySourceLoader` 默认文件 properties/yml 优先级与注释相反：先 `addLast` properties 再 `addLast` yml，导致 properties 覆盖 yml；注释与 Spring 语义均为 yml 覆盖 properties | 同 key 同时出现在 application.properties 与 application.yml 时 | 已修于 M7：yml 先 `addLast`（更高优先级）、properties 后 `addLast`（最低） |
