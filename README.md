...
| 里程碑 | M0–M10 本机落地自验通过；M10 VeriTrail 导入证据复验 15/15 HARD 断言通过（原始冻结坐标 [v0.m10](https://github.com/NoctilumeDev/MiniSpringBoot/releases/tag/v0.m10)，独立审计正确性修订后的当前维护坐标 [v0.m10.2](https://github.com/NoctilumeDev/MiniSpringBoot/releases/tag/v0.m10.2)；账目见 [roadmap](docs/06-roadmap.md)） |
...
- **M10** ✅：Nginx + 3 实例 + 前端 `dist` 同源托管；有界容量、单实例故障切换、事务与 MySQL 就绪演练本机通过；VeriTrail 导入证据复验 15/15 HARD 断言通过（原始冻结发布 [`v0.m10`](https://github.com/NoctilumeDev/MiniSpringBoot/releases/tag/v0.m10)，当前维护发布 [`v0.m10.2`](https://github.com/NoctilumeDev/MiniSpringBoot/releases/tag/v0.m10.2)；全拓扑生命周期所有权仍明确为 `NOT_PROVEN`）
