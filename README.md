...
> **双轨制**：上表是**框架内核**（零第三方依赖，用于教学）。在它之上还有一条「demo 应用」轨道——业务代码 + Vue 前端 + MySQL + Nginx，用它证明内核「真能用」，并跑通全链路、3 实例高可用。详见 [docs/architecture.md](docs/architecture.md)。
...
内核代码必须经得起看，demo 必须真正跑得起来。目标包括：

- 每个模块都有单元测试，核心链路有集成测试
- 并发测试：内嵌服务器线程安全、连接池无泄漏
- 一个能真正跑起来的示例应用 `demo`（前端 + 后端 + 数据库全链路）
