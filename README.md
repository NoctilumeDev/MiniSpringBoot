...
| 内核代码 | 155 个 Java 文件 / 7,811 个物理行（八个内核模块的 `src/main`；运行 `python scripts/verify_repository_contracts.py --metrics` 精确复核） |
| 内核强制传递的第三方运行时依赖 | **0**（HikariCP 是 `autoconfigure` 的直接 optional 集成，不会传递给使用方；demo 显式提供 HikariCP 与 MySQL 驱动） |
| 测试 | 85 个（本地与 [CI](https://github.com/NoctilumeDev/MiniSpringBoot/actions) 云端 MySQL 上均全绿；jdbc 单测真连库） |
...
> 麻雀虽小，五脏俱全。内核的验收标准是「工程化、可复现、边界明确」；demo 的验收标准是「真能用 + 全链路 + 有界多实例演练」。
...
> 推送到 main 或更新 PR 即触发 [GitHub Actions CI](.github/workflows/ci.yml)：backend 在云端起 MySQL service 跑全量 85 个测试（jdbc 单测真连库），frontend 在 Node 22 上执行 `npm ci`、Vite build 与 moderate 级依赖审计——本地能跑的，云端同样验证。
...
- **M10** ✅：Nginx + 3 实例 + 前端 `dist` 同源托管；有界容量、单实例故障切换、事务与 MySQL 就绪演练本机通过；VeriTrail 导入证据复验 15/15 HARD 断言通过（原始冻结发布 [`v0.m10`](https://github.com/NoctilumeDev/MiniSpringBoot/releases/tag/v0.m10)，当前维护发布 [`v0.m10.1`](https://github.com/NoctilumeDev/MiniSpringBoot/releases/tag/v0.m10.1)；全拓扑生命周期所有权仍明确为 `NOT_PROVEN`）
